...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function H2 = get_H_MTF_M(I_MS,sensor,tag,ratio)
...
switch sensor
    ...
    case 'WV2'
     ...
     case 'WV3'
        GNyq = [0.35 .* ones(1,7), 0.27];   %% new add
    case 'none'
        if strcmp(tag,'WV2')
            GNyq = 0.15 .* ones(1,8);
        else
...
for ii = 1 : nBands
    ...
    H2(:,:,ii) = freqz2(h,size(I_MS,1),size(I_MS,2)); % laplase matrix
    H2(:,:,ii) = ifftshift(H2(:,:,ii));
